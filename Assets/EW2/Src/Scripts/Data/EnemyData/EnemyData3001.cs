...
namespace EW2
{
    public class EnemyData3001 : EnemyData
    {
        // just base stats
    }

#if UNITY_EDITOR
    public class Enemy3001Postprocessor : AssetPostprocessor
    {
        static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets,
            string[] movedFromAssetPaths)
        {
            string csvFormat = "Assets/EW2/CSV/Enemies/Enemy_3001";
            foreach (string str in importedAssets)
            {
                if (str.IndexOf(csvFormat, StringComparison.Ordinal) != -1 && str.IndexOf(".csv", StringComparison.Ordinal) != -1)
                {
                    ...
                    // get asset file
                    string nameAsset = nameof(EnemyData3001) + ".asset";
                    string assetFile = "Assets/EW2/Resources/CSV/Units/" + nameAsset;
                    EnemyData3001 gm = AssetDatabase.LoadAssetAtPath<EnemyData3001>(assetFile);
                    if (gm == null)
                    {
                        gm = ScriptableObject.CreateInstance<EnemyData3001>();
