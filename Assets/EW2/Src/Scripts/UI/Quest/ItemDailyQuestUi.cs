...
namespace EW2
{
    public class ItemDailyQuestUi : EnhancedScrollerCellView
    {
        ...
        private void ClaimCLick()
        {
            ...
            reward.AddToUserData(true, AnalyticsConstants.SourceDailyQuest);
        ...
        private void ShowUi()
        {
            var name = Localization.Current.Get("quest", $"quest_daily_{questData.questId}");
            ...
            if (questData.rewards.Length > 0)
            {
                ...
                //TODO doing Daily quest
                // var activityPointItem = ResourceUtils.GetRewardUi(questData.rewards[0].type);
                // activityPointItem.SetData(questData.rewards[0]);
                // activityPointItem.SetParent(panelReward);
