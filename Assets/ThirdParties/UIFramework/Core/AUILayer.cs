...
namespace Zitga.UIFramework
{
    ...
    public abstract class AUILayer<TScreen> : MonoBehaviour where TScreen : IUIScreenController
    {
        ...
        public abstract void HideScreen(TScreen screen, bool animated);
        ...
        /// <param name="animated"></param>
        public void HideScreenById(string screenId, bool animated = true)
        {
            TScreen ctl;
            if (registeredScreens.TryGetValue(screenId, out ctl))
            {
                HideScreen(ctl, animated);
            }
            else
            {
        ...
        public virtual void HideAllPopup(bool animate = true)
        {
            foreach (var uiScreenController in registeredScreens)
            {
                var window = uiScreenController.Value as IWindowController;
                if (window != null)
                {
