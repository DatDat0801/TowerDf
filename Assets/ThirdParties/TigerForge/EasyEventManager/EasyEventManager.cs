...
namespace TigerForge
{
    ...
    public class EventManager
    {
        ...
        public static void StartListening(string eventName, UnityAction callBack, string callBackID = "")
        {
            ...
            if (callBackID != "")
            {
                if (!callBacks.ContainsKey(eventName + "_" + callBackID))
                    callBacks.Add(eventName + "_" + callBackID, callBack);
                else
                    callBacks[eventName + "_" + callBackID] = callBack;
            }
        }
        ...
        /// <param name="eventName"></param>
        /// <returns></returns>
        public static object GetObject(string eventName)
        {
            try
            {
        ...
        public static T GetData<T>(string eventName)
        {
            try
            {
                if (storage.ContainsKey(eventName)) return (T) storage[eventName];
                return default;
            }
            catch (System.Exception)
            {
